 ...
 * ~300KB heap with a single request. 32KB covers 3 modes × 8 valve settings. */
#define WEB_UI_MAX_JSON_BODY_LEN 32768
...
static char s_prop_id[32] = "Valve32Prop";
...
static esp_err_t save_connection_cfg_nvs(void)
{
    ...
    prop_cfg = (char *)calloc(1, VALVE_CONFIG_JSON_MAX);
    if (!prop_cfg) {
        return ESP_ERR_NO_MEM;
    }
    valve_engine_get_config_json(prop_cfg, VALVE_CONFIG_JSON_MAX);
    if (!conn_cfg_lock()) {
        free(prop_cfg);
        return ESP_ERR_TIMEOUT;
    }
    cfg_copy = s_conn_cfg;
    conn_cfg_unlock();
    err = web_ui_json_save_connection_cfg(WEB_UI_CONFIG_FILE_PATH, &cfg_copy, prop_cfg);
    free(prop_cfg);
    return err;
}
...
unauthorized:
    httpd_resp_set_hdr(req, "WWW-Authenticate", "Basic realm=\"PX-Valve-V1\"");
...
static void sanitize_network_name(const char *src, char *out, size_t out_size)
{
    ...
    if (w == 0) {
        strncpy(out, "px-valve-v1", out_size - 1);
        out[out_size - 1] = '\0';
        return;
    }
...
static void build_default_identity(void)
{
    ...
    if (err == ESP_OK) {
        snprintf(s_prop_id, sizeof(s_prop_id), "Valve32Prop");
    }
    ...
    if (s_conn_cfg.network_name[0] == '\0') {
        ...
        if (esp_read_mac(mac, ESP_MAC_WIFI_STA) == ESP_OK) {
            snprintf(network_id, sizeof(network_id), "px-valve-v1-%02x%02x", mac[4], mac[5]);
        } else {
            copy_bounded_local(network_id, sizeof(network_id), "px-valve-v1");
        }
...
static const char *valve_led_hint_name_local(valve_led_hint_t hint);
...
{
    return valve_engine_get_heartbeat_interval_ms();
}
...
static void mqtt_publish_state(bool retained)
{
    ...
    state_json = (char *)malloc(VALVE_STATE_JSON_MAX);
    if (!state_json) {
    ...
    valve_engine_get_state_json(state_json, VALVE_STATE_JSON_MAX);
...
static void mqtt_publish_legacy_heartbeat(void)
{
    ...
    /* generator.js watches /Paradox/Props for {"id":"Valve32Prop"}. */
    snprintf(payload,
             sizeof(payload),
...
static void mqtt_publish_announce(void)
{
    ...
    valve_battery_snapshot_t battery_snap;
    esp_netif_t *sta_netif = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    const esp_app_desc_t *app = esp_app_get_description();
    ...
    valve_engine_get_battery_snapshot(&battery_snap);
    ...
    snprintf(announce,
             ...
             valve_led_hint_name_local(valve_engine_get_led_hint()),
             app->version,
             app->version,
             app->date,
             app->time,
...
static void mqtt_apply_follower_payload(const char *payload, int payload_len)
{
    ...
    valve_engine_get_state_json(local_state_json, sizeof(local_state_json));
    ...
    if (local_remaining_s >= 0 &&
        ...
        if (valve_engine_handle_command_json(cmd, response, sizeof(response)) == ESP_OK) {
    ...
    if (has_pause && game_paused) {
        if (strcmp(local_state, "countdown") == 0) {
            if (valve_engine_handle_command_json("{\"command\":\"pause\"}", response, sizeof(response)) == ESP_OK) {
        ...
        if (valve_engine_handle_command_json("{\"command\":\"resume\"}", response, sizeof(response)) == ESP_OK) {
            mqtt_publish_follow_event("commandOverridden",
                                      local_remaining_s,
                                      remote_remaining_s,
...
static void mqtt_handle_command_payload(const char *payload, int payload_len)
{
    ...
    if (valve_engine_handle_command_json(cmd_json, response, sizeof(response)) == ESP_OK) {
...
static void mqtt_state_task(void *arg)
{
    ...
    while (true) {
        ...
        while (valve_engine_pop_warning_message(warning_msg, sizeof(warning_msg))) {
...
static esp_err_t state_get_handler(httpd_req_t *req)
{
    ...
    payload = (char *)malloc(VALVE_STATE_JSON_MAX);
    if (!payload) {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Out of memory");
        return ESP_ERR_NO_MEM;
    }

    valve_engine_get_state_json(payload, VALVE_STATE_JSON_MAX);
    httpd_resp_set_type(req, "application/json");
...
static void build_unified_config_json(char *out, size_t out_size)
{
    ...
    prop_payload = (char *)calloc(1, VALVE_CONFIG_JSON_MAX);
    if (!prop_payload) {
        snprintf(out, out_size, "{}");
        return;
    }
    valve_engine_get_config_json(prop_payload, VALVE_CONFIG_JSON_MAX);
...
static esp_err_t config_get_handler(httpd_req_t *req)
{
    ...
    payload = (char *)calloc(1, VALVE_CONFIG_JSON_MAX);
    ...
    build_unified_config_json(payload, VALVE_CONFIG_JSON_MAX);
    httpd_resp_set_type(req, "application/json");
...
static esp_err_t config_defaults_get_handler(httpd_req_t *req)
{
    ...
    valve_engine_get_default_config_json(payload, VALVE_CONFIG_JSON_MAX);
    httpd_resp_set_type(req, "application/json");
    esp_err_t err = httpd_resp_send(req, payload, HTTPD_RESP_USE_STRLEN);
    free(payload);
    return err;
...
static esp_err_t command_post_handler(httpd_req_t *req)
{
    ...
    engine_err = valve_engine_handle_command_json(body, response, sizeof(response));
...
static esp_err_t config_post_handler(httpd_req_t *req)
{
    ...
    engine_err = valve_engine_apply_config_json(body, persist, response, sizeof(response));
    if (engine_err == ESP_ERR_TIMEOUT) {
        cJSON_Delete(root);
        free(body);
        return send_json_error(req, "503 Service Unavailable", "busy");
...
static esp_err_t config_restore_post_handler(httpd_req_t *req)
{
    ...
    engine_err = valve_engine_restore_defaults(false, response, sizeof(response));
    if (engine_err == ESP_ERR_TIMEOUT) {
        return send_json_error(req, "503 Service Unavailable", "busy");
    }
    if (engine_err != ESP_OK) {
...
static esp_err_t connection_get_handler(httpd_req_t *req)
{
    ...
    char ap_ssid[33] = "Paradox-PXValveV1";
    connection_cfg_t cfg_snap;
...
static esp_err_t device_details_get_handler(httpd_req_t *req)
{
    ...
    valve_engine_get_state_json(state_json, sizeof(state_json));
...
static esp_err_t ws_handler(httpd_req_t *req)
{
    ...
    if (payload[0] == '\0') {
        ...
        esp_err_t engine_err = valve_engine_handle_command_json(payload, reply_buf, sizeof(reply_buf));
        if (engine_err == ESP_ERR_TIMEOUT) {
...
static void wifi_on_sta_connected(const char *ip_text, void *ctx)
{
    ...
    /* Notify valve engine of WiFi connection for state snapshot RSSI/SSID. */
    svc_wifi_get_status(&wifi_status);
    if (wifi_status.connected) {
        valve_engine_notify_wifi_connected(wifi_status.rssi);
        valve_engine_set_wifi_ssid(wifi_status.ssid);
    }
...
static void wifi_on_sta_disconnected(int reason, const char *reason_text, void *ctx)
{
    ...
    /* Notify valve engine of WiFi disconnection. */
    valve_engine_notify_wifi_disconnected();
...
esp_err_t web_ui_start(void)
{
...

    {
        ...
        snprintf(ap_ssid, sizeof(ap_ssid), "Paradox-PXValveV1-%02X%02X", mac[4], mac[5]);
